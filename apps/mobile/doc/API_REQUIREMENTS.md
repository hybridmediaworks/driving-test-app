...
| Tests (list/detail) | `GET /tests`, `GET /tests/{id}` | 🟡 renamed → `GET /quizzes`, `GET /quizzes/{quiz}` | 🟡 partial | Today tab (`app/(tabs)/index.tsx`) and test intro (`app/test/[id].tsx`) read live quizzes via `services/api/todayService.ts` when the id is numeric. `app/test/see-all.tsx` and the actual quiz-taking/results/review screens still read `data/mockTests.ts` via `services/testService.ts` — next to migrate. |
| Tests — hero card | `GET /tests/hero` | ❌ not built | ✅ (client-derived) | No dedicated endpoint — Today's hero card picks the first unlocked practice quiz from `GET /quizzes`, in category-then-list order. |
| Quiz questions | `GET /tests/{id}/questions` | 🟡 merged into `GET /quizzes/{quiz}` | ❌ | Quiz-taking screen (`app/test/quiz/[id].tsx`) still uses `data/mockQuestions.ts` — not in scope yet |
...
| Exam simulator | `GET /exams/config`, `POST /exams/{id}/simulations` | ❌ not built | 🟡 partial | No dedicated endpoint — Today's Exam card is the first `GET /quizzes?quiz_type=final` result (`services/api/todayService.ts`). The actual "start a simulation" flow (`POST /exams/{id}/simulations`) doesn't exist yet, so tapping through still lands on the mock quiz-taking screen. |
...
| Theory / study material | `GET /theory`, `GET /theory/{id}/download` | 🟡 replaced by `handbooks`, `cheat-sheets` (+download), `flashcards`, `videos` | 🟡 partial | Today tab's Theory section lists live `GET /cheat-sheets` (`services/api/todayService.ts`). Download/detail and `handbooks`/`flashcards`/`videos` aren't wired up yet; `app/theory/see-all.tsx` still uses `MOCK_THEORY_ITEMS`. |

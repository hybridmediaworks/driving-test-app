...
import type { Handbook, State, VehicleType } from "@driving-test-app/shared";
...
export default function EditHandbookPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    api.get<{ handbook: Handbook }>(`/admin/handbooks/${id}`).then((res) => {
      const h = res.handbook;
      setHandbook(h);
      setStateId(String(h.state_id));
      setVehicleTypeId(String(h.vehicle_type_id));
      setLanguage(h.language);
      setTitle(h.title);
      setSourceUrl(h.source_url ?? "");
      setTotalWords(h.total_words != null ? String(h.total_words) : "");
      setChapters(
        h.chapters && h.chapters.length > 0
          ? h.chapters.map((c) => ({
              title: c.title,
              sections:
                c.sections && c.sections.length > 0
                  ? c.sections.map((s) => ({ heading: s.heading ?? "", content: s.content }))
                  : [emptySection()],
            }))
          : [emptyChapter()],
      );
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      await api.post(`/admin/handbooks/${id}`, formData);
    ...
    <AdminGuard>
      <AppLayout
        breadcrumbs={[
          ...
          { title: "Edit", href: "/admin/handbooks" },
        ]}
      >
        <div className="app-page">
          <div className="space-y-0.5">
            <h1 className="text-lg font-semibold">Edit handbook</h1>
            <p className="text-sm text-muted-foreground">{handbook.title}</p>
          </div>
          ...
          <form className="w-full max-w-2xl space-y-6" onSubmit={submit}>
            ...
            <div className="grid gap-2">
              ...
              <Input id="language" maxLength={50} value={language} onChange={(e) => setLanguage(e.target.value)} />
            ...
            <div className="grid gap-2">
              ...
              <Input id="total_words" type="number" min={0} value={totalWords} onChange={(e) => setTotalWords(e.target.value)} />
              <InputError message={errors.total_words?.[0]} />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="pdf">PDF</Label>
              {handbook.pdf_url && !pdf && (
                <a href={handbook.pdf_url} target="_blank" rel="noopener noreferrer" className="text-sm underline hover:text-foreground">
                  View current PDF
                </a>
              )}
              <p className="text-xs text-muted-foreground">Uploading a new file replaces the current one. Max 50 MB.</p>
              <input
                id="pdf"
                type="file"
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">Saving replaces the entire chapter/section list.</p>

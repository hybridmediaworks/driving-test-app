...
import type { CheatSheet, PaginatedResponse, QuizCategory, State, VehicleType } from "@driving-test-app/shared";
...
type CheatSheetsResponse = {
  cheat_sheets: PaginatedResponse<CheatSheet>;
  categories: Pick<QuizCategory, "id" | "name" | "title">[];
  states: State[];
  vehicle_types: Pick<VehicleType, "id" | "name" | "title">[];
};

function CheatSheetsIndexInner() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [res, setRes] = useState<CheatSheetsResponse | null>(null);

  const query = searchParams.toString();

  function load() {
    api.get<CheatSheetsResponse>(`/admin/cheat-sheets${query ? `?${query}` : ""}`).then(setRes);
  }
  ...
  function updateFilter(key: string, value: string) {
    ...
    router.replace(`/admin/cheat-sheets?${params.toString()}`);
  }

  const del = useDeleteConfirm<CheatSheet>(
    (c) => api.delete(`/admin/cheat-sheets/${c.id}`),
    load,
    "Failed to delete cheat sheet.",
  );

  const rows = res?.cheat_sheets.data ?? [];
        ...
        <div className="app-page">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">Condensed study guides by category, state, and vehicle type</p>
            </div>
            <Button className="w-full shrink-0 sm:w-auto" render={<Link href="/admin/cheat-sheets/create" />}>
              New cheat sheet
            </Button>
          </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                All cheat sheets <span className="font-normal text-muted-foreground">({res?.cheat_sheets.meta.total ?? 0})</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {rows.length === 0 ? (
                <div className="text-sm text-muted-foreground">No cheat sheets yet.</div>
              ) : (
                  ...
                  {rows.map((c) => (
                    <li key={c.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex min-w-0 gap-3">
                        {c.cover_image_url && (
                          <div className="shrink-0 overflow-hidden rounded-md border border-border">
                            ...
                            <img src={c.cover_image_url} alt="" className="size-12 object-cover sm:size-16" />
                          </div>
                        )}
                        <div className="min-w-0 space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Order {c.order_no} · {c.sections_count ?? 0} section{c.sections_count === 1 ? "" : "s"} ·{" "}
                            {c.category?.title ?? "Any category"}
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button variant="outline" size="sm" render={<Link href={`/admin/cheat-sheets/${c.id}/edit`} />}>
                          Edit
                        </Button>
              ...
              {res && res.cheat_sheets.meta.total > 0 && <Paginator meta={res.cheat_sheets.meta} />}
            </CardContent>
          </Card>
          ...
          <ConfirmDeleteDialog
            ...
            description={
              del.error ??
              (del.target ? `Are you sure you want to delete "${del.target.title}"? All its sections will be removed. This cannot be undone.` : "")
            }

...
| State (admin) | full CRUD under `/admin/states` (`show` was already there but had no frontend page) | ✅ Wired — list/create/edit pre-existing; new `app/admin/states/[id]/page.tsx` view page composes it with filtered calls to the quizzes/cheat-sheets/flashcards/handbooks/videos admin list endpoints (`?state_id=`) to show everything scoped to that state in one place, with counts and recent-item previews |
| Video (admin) | full CRUD under `/admin/videos` | ✅ Wired — backend (`Admin\VideoController`, routes, resource, factory) existed with zero frontend until this pass; new `app/admin/videos/{page,create,[id]/edit}.tsx`, following the `CheatSheetController`-style envelope (paginated list + category/state/vehicle-type lookup lists). The create/edit forms surface the backend's `external_url` vs `disk`+`path` `required_without` validation as an explicit source-type toggle rather than two silently-optional fields |
| Handbook (admin) | full CRUD under `/admin/handbooks` | ✅ Wired — same "backend-complete, no frontend" gap as Video; new `app/admin/handbooks/{page,create,[id]/edit}.tsx`. One real level deeper than Cheat Sheets' flat `sections` — chapters each own a `sections` array, so the form is a two-level repeater (add/remove chapter, add/remove section within a chapter), with validation errors mapped through `chapters.N.title` / `chapters.N.sections.M.{heading,content}` |
...
- **Backend**: `GET /admin/stats` → `{ users: {total,admins,verified,new_last_7_days,daily_new_last_7_days}, quizzes: {total,active,categories,questions}, attempts: {total,completed,in_progress,average_score,last_7_days,daily_last_7_days}, content: {flashcards:{total,active,premium,reviews}, cheat_sheets:{total,active,premium}}, billing: {...} }`. Admin-only (403 otherwise). `daily_new_last_7_days`/`daily_last_7_days` are zero-filled 7-element arrays (oldest→newest), added to drive the KPI sparklines — see `Admin\StatsController::dailyCounts()`.
- **Frontend**: `components/dashboard/AdminDashboard.tsx` (used from `app/dashboard/page.tsx` when `user.is_admin`). KPI row with sparklines (`Sparkline.tsx`), a content-by-type magnitude comparison (`HorizontalBars.tsx`), an attempt-status part-to-whole bar (`StackedBar.tsx`), a Pass Guarantee claims table (shadcn `Table` + `StatusPill.tsx`), and an engagement KPI row built from the `billing` block.
- **Types**: `packages/shared/src/types/admin.ts` → `AdminStats` (now includes `billing`, which the API already returned but the type previously omitted).

### Stats → Learner Dashboard

- **Backend**: `GET /me/stats` (auth-only, not admin-gated) → `{ attempts: {total,completed,in_progress,passed,average_score,last_7_days,recent_scores}, flashcards: {total_active,known,unknown}, cheat_sheets: {total_active}, categories: [{id,name,average_score,attempts_count}] }`, all scoped to `$request->user()`. `recent_scores` is the last 10 completed attempts' scores, oldest→newest, for the trend chart. `categories` is a `GROUP BY quiz_categories.id` average over the user's own completed attempts (quizzes always have a category — the FK is non-nullable — so no "uncategorized" bucket exists). Controller: `App\Http\Controllers\Api\V1\StatsController` (aliased `AdminStatsController` for the admin one in `routes/api.php` to avoid a naming clash).
...
- **Frontend**: `components/dashboard/LearnerDashboard.tsx` — a pass-rate `RadialMeter`, a KPI row, the interactive `ScoreTrendChart` (crosshair + tooltip + "view as table" toggle), a `StackedBar` for flashcard known/unknown/not-started, `HorizontalBars` for per-category performance (flags the weakest category), and a recent-activity list fed by the existing `GET /attempts?per_page=5` (the controller floors `per_page` at 5, so this is the effective minimum, not an arbitrary choice).
- **Types**: `packages/shared/src/types/stats.ts` → `UserStats`.
- **Chart tokens**: `app/globals.css`'s `--chart-1..5` were previously an unused grayscale placeholder (shadcn boilerplate, never referenced anywhere). They're now a real categorical palette (slot 1 pinned to the app's own `--blue-primary`), plus `--chart-wash`/`--chart-track`/`--chart-grid` and `--status-good/warning/critical` — all colorblind- and contrast-validated per-mode (see the dataviz skill's `validate_palette.js`; re-run it before changing any of these hexes).
...
- **Auth state**: `lib/auth-context.tsx`'s `useAuth()` — `user`, `loading`, `login/register/logout`. **Don't call `logout()` directly from a component** — every "Log out" trigger (account dropdown, verify-email page, post-account-deletion) goes through `useLogout()` in the same file instead, which wraps it with a `pending` flag (blocks double-fire, lets the button show "Logging out…") and a consistent redirect (`/login` by default, overridable — e.g. `DeleteUser.tsx` passes `"/"`). Added because the three call sites had each grown a different post-logout behavior: one relied on `AppLayout`'s route guard noticing `user` went null, one didn't redirect at all, one redirected manually.
...
- **Nav**: `components/app/AppSidebar.tsx`'s `platformNavItems`/`adminNavItems` (rendered as separate "Platform"/"Admin" `NavMain` groups, the latter only for `user.is_admin`) — add new top-level app-shell pages here.

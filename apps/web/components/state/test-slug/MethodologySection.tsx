...
import type { ExpertSummary, Handbook, PaginatedResponse, State } from "@driving-test-app/shared";
...
function toReviewer(expert: ExpertSummary, index: number): Reviewer {
  return {
    name: expert.name,
    role: expert.role_label ?? expert.title,
    quote:
      "The questions hit the same reasoning patterns we use on the actual exam. Students who practice here walk in prepared.",
    photo: expert.photo_url ?? PLACEHOLDER_PHOTOS[expert.slug] ?? DESIGN_REVIEWERS[index]?.photo ?? "",
...
export default function MethodologySection() {
  ...
  const reviewers = experts.length >= 2 ? experts.slice(0, 2).map(toReviewer) : DESIGN_REVIEWERS;
      ...
      <div className="mx-auto flex max-w-container flex-col items-center gap-15">
        <div className="flex max-w-205 flex-col items-center gap-6 text-center">
          ...
          <Paragraph size="lg" className="max-w-153.5 text-center">
            Real people verify every state’s questions against the official manual — so what you practice matches what
            you’ll be tested on.
          </Paragraph>
        </div>
        ...
        <div className="grid w-full grid-cols-1 items-stretch gap-5 lg:grid-cols-[minmax(0,325fr)_minmax(0,670fr)_minmax(0,325fr)]">
          ...
          <div className="order-first flex flex-col justify-between gap-8 rounded-[32px] bg-[linear-gradient(118deg,#0d142c_0%,#172554_100%)] p-8 drop-shadow-[0px_30px_32px_rgba(16,24,40,0.3),0px_8px_9px_rgba(16,24,40,0.07)] lg:order-none lg:p-10">
            <div className="space-y-4">
              <h3 className="font-sora text-3xl leading-11 font-semibold text-white">How we build these questions</h3>
              <div className="space-y-5">
                {methods.map((method) => (
                  <div key={method.title} className="flex items-start gap-4">
                    <span className={`flex size-6.5 shrink-0 items-center justify-center rounded-lg ${method.tone}`}>
                    ...
                    <div className="flex-1 space-y-1.5">
                      ...
                      <p className="text-sm leading-6 text-white/60">{method.body}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-wrap items-end justify-between gap-6 border-t border-white/12 pt-4">
              <div className="flex flex-col justify-center gap-3.5">
                <div className="flex gap-0.5" aria-hidden="true">
                  {Array.from({ length: TRUSTPILOT_MAX }).map((_, i) => (
                    <Star key={i} className="size-4.5 fill-yellow-500 text-yellow-500" strokeWidth={0} />
                  ))}
                </div>
              ...
              <p className="max-w-71.75 text-[13px] leading-6 text-white/60">
                ...
                {handbook ? (
                  <Link href={`/handbook/${handbook.id}`} className="text-white underline">
                    {handbook.title}
                  </Link>
                ) : (
                  <span className="text-white">the official {selectedState} driver handbook</span>
                )}
...
function ReviewerCard({ reviewer }: { reviewer: Reviewer | undefined }) {
  ...
  const body = (
    <>
      ...
      <img src={reviewer.photo} alt="" className="h-61 w-full rounded-3xl object-cover" />
      <div className="flex flex-col gap-6 px-2">
        <Paragraph>“{reviewer.quote}”</Paragraph>
        <div>

...
## Status: implemented (M1–M9 complete, plus post-M9 additions)

Every milestone in §6 is built and backend-verified (157/157 automated tests at M9, 225/225 as of the post-M9 additions below, including signed Stripe webhook simulation) and frontend-verified (clean typecheck/lint across every new file, plus a real browser walkthrough of the guest → free-user paywall path). See **§8 "Implementation notes"** below for where the real build diverged from this plan — including four changes made *after* M9 shipped (the Free plan card was removed, a free trial was added to Weekly, list/browse endpoints were fixed to expose real per-user lock status instead of just the content's own premium flag, and edit/delete were locked down for any plan already synced to Stripe) — and **§9 "Testing steps"** for exactly how to re-verify it, including the one thing that still needs a human: actually completing a Stripe test-mode checkout in a browser.
...
| Weekly subscriber (active or trialing)‡ | Yes | Yes | Yes | Yes | No | Yes | Yes |
...
‡ Weekly is the only plan with a trial (7 days, card required — see §8 item 9). No separate row needed: Cashier's `Subscription::active()` doesn't exclude `stripe_status = 'trialing'`, so a trialing subscription already satisfies the same `active()` check as a paying one — full access from the moment of checkout, not just after the trial ends.
...
`id, key (unique: weekly|monthly|lifetime_family — no "free" row, see §8 item 8), name, type (recurring|one_time), billing_interval (week|month|null), price_cents, trial_days (nullable, see §8 item 9), stripe_price_id (nullable), stripe_product_id (nullable), max_seats (default 1, 3 for lifetime_family), is_active, sort_order, timestamps`
...
**Post-M9 additions (not in the original M1–M9 plan above):**

8. **The Free plan was removed** — `plans.key = 'free'` no longer exists (deleted from the DB and from `PlanSeeder`). This was a product decision, not a bug fix: `EntitlementTier::Free` and the "free registered user" row in §1's matrix are untouched and still real — a registered user with no subscription still gets that tier — there's just no longer a purchasable/free-to-click card for it on `/pricing`. Checked before deleting: no `family_groups` or `subscriptions` referenced the row. **If you ever delete another plan row, always check for real subscriber/family references first** — `EntitlementResolver::tierForStripePrice()` silently falls back to `EntitlementTier::Free` for any active subscription whose `stripe_price` no longer matches a `plans` row, which would strand a paying customer's premium access with no error or log signal.
9. **A free trial was added to the Weekly plan** — 7 days, card required at checkout (Cashier's `SubscriptionBuilder::trialDays()`, applied in `CreateCheckoutSession` whenever `plan->trial_days` is set), auto-converts to a paid subscription unless canceled first. `plans.trial_days` (nullable int) is the only new column; no `EntitlementResolver` changes were needed at all — confirmed by reading Cashier's source directly that `Subscription::active()` doesn't exclude `stripe_status = 'trialing'`, so a trialing subscription already satisfies the same active-subscriber check a paying one does. `trial_days` is admin-editable per plan (`StorePlanRequest`/`UpdatePlanRequest`), and `BillingController::subscription()` now also exposes `on_trial`/`trial_ends_at` so `/settings/billing` can show a real "trial ends on X" banner instead of a bare "Status: active" that would otherwise say nothing about an upcoming first charge.
10. **List/browse endpoints were fixed to expose real per-user lock status.** `QuizResource`/`CheatSheetResource`'s `index()` output previously only ever returned the item's own `is_premium` flag — never a per-user `locked` field — even though `FlashcardResource` already computed one correctly (`Gate::forUser($request->user('sanctum'))->allows(...)`, same pattern `show()` already used for both resources). Net effect: an actively-subscribed user browsing `/quizzes`, the phase-ladder step grid, or `/cheat-sheets` saw the *exact same* "Premium"/locked-looking UI as a guest, even though clicking into any single item correctly unlocked it — a real, live bug, not a hypothetical one (reproduced against a real paying test account). Fixed by adding `locked` to both resources' output, matching `FlashcardResource`'s existing pattern, and updating every frontend spot that had been inferring lock status from `is_premium` alone (`lib/phaseLadder.ts` → `StepCard.tsx`'s blur/upgrade overlay, `QuizCard.tsx`, the `/cheat-sheets` browse card).
11. **Plan edit/delete is now locked down once a plan is synced to Stripe.** `UpdatePlanRequest` rejects (422) changes to `price_cents`/`billing_interval`/`type` once `stripe_price_id` is set — Stripe Prices are immutable, so "editing" one of those fields in our own table could only ever desync the display from what's actually charged, never really change it. The fix for a real price change is: create a new plan (starts with `stripe_price_id = null`, so `billing:sync-plans` provisions it automatically), deactivate the old one. The delete guard (`stripe_price_id !== null` or has `family_groups` → 422) already existed from M1–M9; this just closes the equivalent gap on the edit path. The admin edit page (`apps/web/app/admin/plans/[id]/edit/page.tsx`) now visibly disables those three fields once synced, rather than letting an admin submit a change only to have it silently do nothing to Stripe.
...
```bash
cd apps/api && php artisan test                 # full backend suite, 225+ tests
...
- `apps/api/tests/Unit/EntitlementResolverTest.php` — full access-matrix (guest/free/weekly/monthly/trialing/family owner+member/past-due grace/expired/admin).
- `apps/api/tests/Feature/Quiz/QuizEntitlementGatingTest.php`, `Flashcard/FlashcardBrowsingTest.php`, `CheatSheet/CheatSheetBrowsingTest.php` — HTTP-level premium gating including a real active-subscriber case, not just admin bypass; also covers the list/browse-endpoint `locked` field (§8 item 10), not just the detail endpoint.
...
- `apps/api/tests/Feature/Billing/{BillingControllerTest,FamilyControllerTest,PassGuaranteeClaimControllerTest}.php` — `BillingControllerTest` also covers the `on_trial`/`trial_ends_at` fields (§8 item 9). `tests/Feature/Admin/PassGuaranteeClaimAdminTest.php`, `tests/Feature/Admin/StatsTest.php`.
- `apps/api/tests/Feature/Admin/PlanManagementTest.php` — plan CRUD including the money-field edit lock and delete guard for Stripe-synced plans (§8 item 11).
- Genuinely untested by the automated suite (real Stripe network calls with no interceptable seam — see Action/controller container-swap pattern used for `CreateCheckoutSession`/`IssueRefund` where one exists): `Subscription::cancel()`'s actual Stripe update, `billingPortalUrl()`'s actual Stripe call, and `trialDays()` actually reaching Stripe on a real checkout session. Covered by the manual walkthrough below instead.
...
3. As a **guest**: visit `/pricing` — confirm real plan cards (Weekly/Monthly/Lifetime Family — no Free card, see §8 item 8) with live prices, not hardcoded ones, and that Weekly's card advertises the 7-day trial. Visit a `is_premium=true` quiz — confirm the locked paywall renders, not the questions. Also confirm the phase-ladder step grid and `/quizzes`/`/cheat-sheets` browse lists show premium items as locked for a guest (§8 item 10).
4. **Register** a free account, verify email (Mailtrap inbox, since `MAIL_MAILER=smtp` points there). Confirm the same premium quiz is still locked for a logged-in free user, and that browse lists (not just the detail page) reflect that too.
...
7. Revisit the premium quiz — confirm it's now unlocked (full questions returned, not the locked teaser), **and** revisit `/quizzes`, `/cheat-sheets`, and a state's phase-ladder page — confirm premium items now show unlocked there too, not just on the detail page (§8 item 10; this is the part that was broken before the fix).
...
10. **Free trial + checkout-resume**: as a fresh guest (logged out, different email), click **Start 7-Day Free Trial** on Weekly's card — confirm redirect to Stripe Checkout showing a trial period before the first charge (not an immediate charge). Separately, test the auth-detour path: start the same checkout while logged out, get redirected to `/login`, click through to `/register` instead, sign up — confirm you land back in Stripe Checkout automatically rather than on the dashboard or the email-verification reminder. After completing checkout, visit `/settings/billing` — confirm it shows a "trial ends on X" banner, not a bare "Status: active".
11. **Lifetime-Family**: from a second (or the same) account, purchase Lifetime Family. Confirm `/settings/billing` shows a seat roster with yourself as owner. Invite a second email — confirm a Mailtrap email arrives with a `/family/claim/{token}` link. From a **different** logged-in account, open that link and confirm the seat claims successfully and that account now has premium access. As the owner, revoke that seat and confirm the revoked account loses premium access on its next request (no caching — entitlement is resolved fresh every request).
12. **Pass Guarantee**: as a premium user, complete a `quiz_type=final` (Exam Simulator) quiz attempt, then visit `/pass-guarantee` — confirm eligibility flips to true and the claim form appears. Submit a claim. As an **admin**, visit `/admin/pass-guarantee-claims`, approve it, then issue the refund — confirm the status flow (`submitted` → `approved` → `refunded`) and that a real refund appears against the original charge in the Stripe test Dashboard.
13. **Plan edit/delete guards**: in `/admin/plans`, edit the synced Weekly plan — confirm price/billing-interval/type are disabled in the UI, and that attempting to change them via a raw API call gets a 422. Confirm the Delete button on a synced plan surfaces the "has real billing history" error rather than deleting it.
14. **Cross-check**: at any point, confirm `GET /v1/billing/subscription` and the `entitlement` block on `GET /v1/me` report the same tier/status — they read the same `EntitlementResolver` call, so a mismatch means a caching bug got introduced somewhere.
